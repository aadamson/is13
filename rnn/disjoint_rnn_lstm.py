...
class model(object):
    ...
    def s_t(self, forward_h_t_i, back_h_t_i):
        s = T.reshape(T.dmatrix(), (numclasses, 2))
        for i in self.numclasses:
            s[i] = T.nnet.softmax(T.dot(forward_h_t_i, self.forward_U[i])
                                  + T.dot(back_h_t_i, self.back_U[i]) 
                                  + self.c[i])[:,0,:]

        return s
    
    def __init__(self, nh, ne, depth, embeddings, numclasses, lam=0.03, adagrad=False, ):
        f = relu
        '''
        ...
        numclasses :: number of non-mutually-exclusive classes to classify examples into
        '''
        # Hyperparameters
        ...
        s, _ = theano.scan(fn=self.s_t, sequences=[forward_h_im1, back_h_im1])
